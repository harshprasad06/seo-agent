...
- Start with frontmatter (title, description, date, keywords, author)
...
// ── Publish approved post via GitHub PR ───────────────────────────────────────

export async function publishBlogPost(postId: string): Promise<string> {
  ...
  const prUrl = await createPullRequest({
    ...
    body: `## SEO Blog Post\n\n**Keyword:** ${post.target_keyword}\n**File:** \`${filePath}\`\n\nGenerated by SEO Agent. Review and merge to publish on ${SITE_DOMAIN}.`,
    filePath,
  ...
  await supabaseAdmin
    .from('blog_posts')
    .update({ status: 'approved', pr_url: prUrl, updated_at: new Date().toISOString() })
    .eq('id', postId);
